...
// POST /api/admin/billing-agent/stop - Stop the billing agent
export async function POST() {
  try {
    ...
    try {
      ...
      console.error('Error in billing agent stop operation:', agentError);
      return NextResponse.json({ 
        error: agentError instanceof Error ? agentError.message : 'Failed to stop billing agent',
        details: 'Check server logs for more information'
      }, { status: 500 });
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Failed to stop billing agent';
    console.error('Error stopping billing agent:', error);
