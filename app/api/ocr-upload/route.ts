  ...
  try {
    ...
    const report = await prisma.report.create({
      data: {
        ...
        testDate: reportData.patientInfo?.collectionDate 
          ? new Date(reportData.patientInfo.collectionDate) 
          : new Date(),
        labName: sanitizedMetadata.labName || 'Extracted from image',
      }
    });

    // Save biomarkers with enhanced error handling
    const biomarkers = [];
    let savedCount = 0;
        ...
        try {
          ...
          const biomarker = await prisma.biomarker.create({
            data: {
              ...
              flag: test.flag || null,
              description: '',
              category: test.category || 'EXTRACTED_FROM_IMAGE'
            }
          });
          ...
          console.error(`Failed to save biomarker ${test.name}:`, error);
          // Continue with other biomarkers
        }
      }
    }

    console.log(`Saved ${savedCount} out of ${reportData.tests?.length || 0} biomarkers for report ${report.id}`);

    // Add data review capabilities
