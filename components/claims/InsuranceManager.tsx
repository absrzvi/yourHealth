...
export function InsuranceManager() {
  ...
  const checkEligibility = async (planId: string) => {
    try {
      ...
      if (response.ok) {
        alert(`Eligibility Check Result:\nActive: ${data.isActive ?? data.isEligible}\nDeductible: $${data.deductible ?? data.coverage?.deductible ?? 'N/A'}`);
      } else {
        alert(`Eligibility check failed: ${data.error || 'Unknown error'}`);
      }
    } catch (error: any) {
      alert('Eligibility check failed: ' + (error.message || error));
    }
  };
