...
const EnhancedChatInterface: React.FC<EnhancedChatInterfaceProps> = ({
  ...
  const handleSendMessage = async () => {
    ...
    // Create a temporary ID for the AI's response
    const responseId = `temp-${Date.now()}`;
    
    try {
      // First try to generate a visualization if the message is a visualization request
      const isVisualization = await generateVisualization(messageText);
      
      if (isVisualization) {
        // Update message status and return early - the effect will handle the visualization
        setMessages(prev => 
          prev.map(msg => 
      ...
      // If not a visualization, proceed with text response
        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            ...
            try {
              ...
              if (parsed.done) {
                // Update message status to sent when done
                setMessages(prev => 
                  prev.map(msg => 
                    msg.id === responseId 
                      ? { ...msg, status: 'sent' as const } 
  ...
  useEffect(() => {
    if (isGeneratingVisualization || !visualizationData || !visualizationType) return;
    
    // Check if we already have a visualization message for this data
    const hasExistingVisualization = messages.some(
      msg => msg.type === visualizationType && 
            JSON.stringify(msg.content) === JSON.stringify(visualizationData)
    );
    
    if (!hasExistingVisualization) {
      ...
      // Clear the visualization data after adding it to the chat
      clearVisualization();
    }
  }, [isGeneratingVisualization, visualizationData, visualizationType, messages, clearVisualization]);
